...
const handler = async (m, {conn, command, usedPrefix}) => {
  if (!db.data.chats[m.chat].modohorny && m.isGroup) throw 'https://whatsapp.com/channel/0029Vajvy2kEwEjwAKP4SI0x *[鉂楌潗堭潗嶐潗咅潗庘潡] The +18 commands are disabled in this group if you are admin and want to activate them use the command #enable modohorny*';
  switch (command) {
    case 'pack':
      const url = await pack[Math.floor(Math.random() * pack.length)];
      conn.sendMessage(m.chat, {image: {url: url}, caption: `_馃サ Pack 馃サ_`}, {quoted: m});
      break;
    case 'pack2':
      const url2 = await packgirl[Math.floor(Math.random() * packgirl.length)];
      conn.sendMessage(m.chat, {image: {url: url2}, caption: `_馃サ Pack 馃サ_`}, {quoted: m});
      break;
    case 'pack3':
      const url3 = await packmen[Math.floor(Math.random() * packmen.length)];
      conn.sendMessage(m.chat, {image: {url: url3}, caption: `_馃サ Pack 3 馃サ_`}, {quoted: m});
      break;
    case 'videoxxx': case 'v铆deoxxx':
      const url4 = await videosxxxc[Math.floor(Math.random() * videosxxxc.length)];
      await conn.sendMessage(m.chat, {video: {url: url4}, caption: `*uuuh! naughty馃サ*`}, {quoted: m});
      break;
    case 'videoxxxlesbi': case 'videolesbixxx': case 'pornolesbivid': case 'pornolesbianavid': case 'pornolesbiv': case 'pornolesbianav': case 'pornolesv':
      ...
      await conn.sendMessage(m.chat, {video: {url: url5}, caption: `*uuuh! naught馃サ*`}, {quoted: m});
      break;
  }
};
handler.command = /^(pack|pack2|pack3|videoxxx|v铆deoxxx|videoxxxlesbi|videolesbixxx|pornolesbivid|pornolesbianavid|pornolesbiv|pornolesbianav|pornolesv)$/i;
